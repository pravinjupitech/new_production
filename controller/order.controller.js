...
export const updateCreateOrder = async (req, res, next) => {
  try {
    ...
    if (!order) {
      ...
      return res
        .status(400)
        .json({ message: "this order not updated", status: false });
    ...
    for (const newOrderItem of newOrderItems) {
      const oldOrderItem = oldOrderItems.find(
        (item) =>
          item.productId.toString() === newOrderItem.productId.toString()
      );
      if (oldOrderItem) {
        const quantityChange = newOrderItem.qty - oldOrderItem.qty;
        if (quantityChange !== 0) {
          const product = await Product.findById({
            _id: newOrderItem.productId,
          });
          if (product) {
            ...
            console.error(
              `Product with ID ${newOrderItem.productId} not found`
            );
          }
        }
      }
...
export const DebitorCalculate = async (req, res, next) => {
  try {
    ...
    currentSales = salesOrderCurrentMonth.reduce(
      (sum, item) => sum + item.grandTotal,
      0
    );
      ...
      if (product) {
        ...
        if (warehouse) {
          ...
          if (pro.currentStock < 0) {
            return res
              .status(404)
              .json({ message: "Product Out Of Stock", status: false });
          }
          // pro.pendingStock -= (orderItem.qty)
          await warehouse.save();
          await product.save();
