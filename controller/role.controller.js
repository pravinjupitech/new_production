...
export const CreatRole = async (req, res, next) => {
  try {
    ...
    return res
      .status(200)
      .json({ Role: role, message: "Role Creation successful", status: true });
  } catch (err) {
    console.log(err);
    return res
      .status(500)
...
export const getRole = async (req, res, next) => {
  try {
    ...
    const roles = await Role.find({ database: database }).populate({
      path: "createdBy",
      model: "user",
    });
...
export const getRoleById = async (req, res, next) => {
  try {
    const getRole = await Role.findById({ _id: req.params.id }).populate({
      path: "createdBy",
      model: "role",
    });
...
export const updatedRole = async (req, res, next) => {
  try {
    ...
    if (req.body.position) {
      return res
        .status(400)
        .json({ message: "this is not valid request", status: false });
    }
    ...
    return res
      .status(200)
      .json({ message: "Account updated successfully", status: true });
  } catch (err) {
    console.error(err);
    return res
      .status(500)
...
export const updatedRoleGloble = async (req, res, next) => {
  try {
    const roles = await Role.find({ roleName: req.body.roleName }).sort({
      sortorder: -1,
    });
    if (!roles.length > 0) {
      return res.status(404).json({ error: "role not found", status: false });
...
export const viewTab = async (req, res, next) => {
  try {
    const tab = await Tab.findOne({ userId: req.params.id }).sort({
      sortorder: -1,
    });
    return tab
      ? res.status(200).json({ Tab: tab, status: true })
      : res.status(404).json({ message: "Tab Not Found", status: false });
  } catch (err) {
    console.log(err);
    return res
      .status(500)
...
export const saveDashboardTabs = async (req, res, next) => {
  try {
    ...
    if (user) {
      for (let item of req.body.tab) {
        const existingId = await user.tab.find(
...
export const viewDashboardTab = async (req, res, next) => {
  try {
    const tab = await DashboardTab.findOne({ userId: req.params.id }).sort({
      sortorder: -1,
    });
    return tab
      ? res.status(200).json({ Tab: tab, status: true })
...
export const deleteRole = async (req, res, next) => {
  try {
    ...
    if (!database) {
      return res
        .status(400)
        .json({ message: "Database field is required", status: false });
    }
